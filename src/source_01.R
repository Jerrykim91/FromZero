...
c(1, 2, 3, c(4:6)) 	# 1~3, 4~6 결합 -> 1 2 3 4 5 6
...
students <- read.table("C:/data/students1.txt", header = T) 

# 파일 마지막 행에서 [Enter]를 누른 경우
students <- read.table("C:/data/students.txt",  header = T) 
students
...
# 파일을 있는 형태 그대로 읽음
students <- read.table("C:/data/students.txt", header = T, as.is = T) 
str(students)

# 파일을 읽을 때 문장을 요인으로 인식하지 않도록 설정
students <- read.table("C:/data/students.txt", header = T, stringsAsFactors = F) 
str(students)


#read.csv 함수: CSV(Comma-Separated Values) 파일을 읽을 때 사용
# 첫 행이 header이므로 header 옵션을 지정할 필요가 없음
students <- read.csv("C:/data/students.csv") 
students
...
students <- read.csv("C:/data/students.csv", stringsAsFactors = FALSE) 
str(students)

#?write.table
# 문장에 큰따옴표가 표시됨.
write.table(students, file = "C:/data/output1.txt") 

# "C:/data/output.txt" 파일 확인해보기
# 문장에 큰따옴표되지 않음.
write.table(students, file = "C:/data/output1.txt", quote = F) 
# "C:/data/output.txt" 파일 확인해보기
...
## (p59) ##
x <- c(-5:5)
options(digits = 3)		# 숫자 표현 시 유효자릿수를 3자리로 설정 -> 출력되는 수들의 자리수가 바뀐다.
sqrt(x) # 제곱근 계산 
...
### ifelse는 if문의 한계를 해결 할 수있다.
### 벡터 연산(각 요소별로 조건 검사)가 가능하다. 
### 단점은 주어진 값에 따라 Y or N 반환 
### 리턴값만 반환 -> 오로지 출력만 가능 -> 조건별 명령어 수행은 불가능하다. 
...
avocado <- read.csv("C:/data/avocado.csv", header=TRUE, sep = ",")
...
#모델링을 위한 가공
# 데이터 프레임 읽기
wine <- read.table("C:/data/wine.data.txt", header = TRUE, sep = ",")

head(wine)

n <- readLines("C:/data/wine.name.txt")
n
...
## (p99) ##
# 데이터 구조 변경
elec_gen <- read.csv("C:/data/electricity_generation_per_person.csv", header = TRUE, sep = ",")
names(elec_gen)
...
elec_use <- read.csv("C:/data/electricity_use_per_person.csv", header = TRUE, sep = ",")
