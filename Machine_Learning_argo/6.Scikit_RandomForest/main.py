...
def step2_learnig():
    ...
    machine_learing = RandomForestClassifier(criterion='entropy', n_estimators=10, max_depth=3,n_jobs=2, random_state=0)
    ...
    # mae
    # mse
    # over fitting인지 아닌지 구분 

    # 학습이 완료된 객체를 지정
    with open('./Machine_Learning_argo/6.Scikit_RandomForest/ml.dat', 'wb') as fp :
        pickle.dump(sc, fp)
...
def step3_using():
    ...
    # 학습 완료된 객체를 복원
    with open('./Machine_Learning_argo/6.Scikit_RandomForest/ml.dat', 'rb') as fp :
        sc = pickle.load(fp)
        ml = pickle.load(fp)
